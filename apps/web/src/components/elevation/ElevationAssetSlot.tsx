...
import { Asset, assetBg, assetSlotText, normalizeRackSide, stripeBg } from "../../lib/infrastructure"
    ...
    <Tooltip title={`${asset.name} · ${asset.assetType}${asset.manufacturer ? ` · ${asset.manufacturer}` : ""}${retired ? " · retired — awaiting removal" : ""}`} placement="right" arrow>
      <Box
        ...
        sx={{
          ...
          bgcolor: assetBg(asset.assetType, mode),
          border: isSelected ? "2px solid #2563eb" : `1px solid ${isDark ? "rgba(255,255,255,0.08)" : "rgba(0,0,0,0.08)"}`,
      ...
      >
        <Box sx={{ flex: 1, display: "flex", flexDirection: "column", justifyContent: "center", px: "7px", overflow: "hidden" }}>
          <Typography sx={{ fontSize: 10.5, fontWeight: 700, color: text.title, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
            {asset.name}
          </Typography>
          {h > 1 && asset.modelNumber ? (
            <Typography sx={{ fontSize: 9, color: text.subtitle, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
              {asset.modelNumber}
            </Typography>
          ) : null}
        </Box>
