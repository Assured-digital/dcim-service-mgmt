...
function applyViewFilter(risks: Risk[], view: string) {
  const now = new Date()
  if (view === "urgent") return risks.filter((r) => deriveRag(r.likelihood, r.impact) === "RED")
  if (view === "review_due") {
    return risks.filter((r) => r.reviewDate && new Date(r.reviewDate) <= now && r.status !== "CLOSED")
  }
  if (view === "assigned") {
    // Placeholder deterministic mapping until user-level ownership is introduced for risks.
    return risks.filter((r) => r.status === "UNDER_REVIEW" || r.status === "MITIGATING")
  }
  return risks
}

export default function RisksIssuesRisksListPage() {
    ...
    <Box>
      <Card>
        ...
        {!isLoading && !error && filtered.length === 0 ? (
          <Box sx={{ p: 2 }}>
            <EmptyState title={filterStatus === "ALL" ? "No risks logged" : `No ${RISK_STATUS_LABELS[filterStatus]?.toLowerCase()} risks`} detail="Log a risk to get started." />
          </Box>
        ) : null}
            ...
            <Table>
              ...
              <TableBody>
                {filtered.map(r => {
                  ...
                  return (
                    <TableRow key={r.id} hover onClick={() => navigate(`/risks-issues/risks/${r.id}`)} sx={{ cursor: "pointer" }}>
