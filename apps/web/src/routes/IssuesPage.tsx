...
export default function IssuesPage() {
  ...
  ISSUE_STATUSES.slice(0, -1).forEach(s => { counts[s] = all.filter(i => i.status === s).length })
  ...
  async function handleCreate() {
    ...
    try {
      ...
      setLogOpen(false)
      setTitle(""); setDescription(""); setSeverity("AMBER"); setReviewDate("")
        ...
        {!isLoading && !error && filtered.length === 0 ? (
          <Box sx={{ p: 2 }}>
            <EmptyState
              ...
              detail="Log an issue to get started." />
          </Box>
        ) : null}
        ...
        {filtered.length > 0 ? (
          <TableContainer>
            <Table>
              ...
              <TableBody>
                {filtered.map(i => (
                  <TableRow key={i.id} hover onClick={() => navigate(`/issues/${i.id}`)} sx={{ cursor: "pointer" }}>
